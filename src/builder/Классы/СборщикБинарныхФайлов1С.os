...
Процедура ВыполнитьКомандуЗапускаАвтономногоСервера(ПараметрыЗапуска)
	...
	//Текст.ДобавитьСтроку("chcp 1251");
	Текст.ДобавитьСтроку("chcp 65001 | """+ИмяФайлаУтилитыАвтономногоСервера+""" "+СтрСоединить(ПараметрыЗапуска," ")+"--db-path="""+ИмяКаталогаВременнойБазы+"""");
	ИмяФайлаЗапуска = МенеджерВременныхФайлов.СоздатьФайл("bat");
	Текст.Записать(ИмяФайлаЗапуска, КодировкаТекста.UTF8);

	Команда = Новый Команда;
	Команда.ДобавитьЛогВыводаКоманды(Лог);
	Команда.УстановитьКоманду(ИмяФайлаЗапуска);
