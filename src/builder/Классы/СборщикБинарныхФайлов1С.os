...
Перем СборкаЧерезУтилитуАвтономногоСервераВнешнихОбработок;
...
Процедура УстановитьПутьКПлатформе1С(НовыйПуть) Экспорт
	...
	СборкаЧерезУтилитуАвтономногоСервераВнешнихОбработок = ФайлУтилиты.Существует();
...
Процедура СформироватьБинарныйФайлОбработки(ИмяИсходногоФайлаОбработки, ИмяФайлаОбработки) Экспорт
	Если СборкаЧерезУтилитуАвтономногоСервераВнешнихОбработок Тогда
		АвтономныйСформироватьФайлОбработкиИзФайлов(ИмяИсходногоФайлаОбработки, ИмяФайлаОбработки);
	Иначе
		Конфигуратор = ПодготовленныйКонфигуратор();
		ПараметрыЗапуска = Конфигуратор.ПолучитьПараметрыЗапуска();
...
Процедура ВыполнитьКомандуЗапускаАвтономногоСервера(ПараметрыЗапуска)
	...
	Если ОбщиеМетоды.ЭтоWindows() Тогда
		КомандныйФайл.ДобавитьКоманду("chcp 65001");
	
		//Приложение = "C:\Program Files\Git\bin\bash.exe";
		//СтрокаЗапуска = "/C ""%1""";
		СтрокаЗапуска = """%1""";
	Иначе
		Приложение = "sh";
	...
	СтрокаЗапускаАвтономного = СтрШаблон("%1 %2 --db-path=%3", """" + ИмяФайлаУтилитыАвтономногоСервера + """", СтрСоединить(ПараметрыЗапуска, " "), """"+ИмяКаталогаВременнойБазы+"""");
	КомандныйФайл.ДобавитьКоманду(СтрокаЗапускаАвтономного);


	КомандныйФайл.УстановитьКодировкуВывода(КодировкаТекста.UTF8);
	КомандныйФайл.ПоказыватьВыводНемедленно(Истина);
	ПутьКИсполняемомуФайлу= КомандныйФайл.Закрыть();
	
	Текст = Новый ТекстовыйДокумент;
	Текст.ДобавитьСтроку("chcp 65001");
	Текст.ДобавитьСтроку(СтрокаЗапускаАвтономного);
	Текст.Записать(ПутьКИсполняемомуФайлу, КодировкаТекста.UTF8);
	...
	//Команда.ДобавитьПараметр(СтрШаблон(СтрокаЗапуска, ПутьКИсполняемомуФайлу));
...
Процедура АвтономныйСформироватьФайлОбработкиИзФайлов(ИмяИсходногоФайлаОбработки, ИмяФайлаОбработки)
	ПараметрыЗапуска = Новый Массив;
	ПараметрыЗапуска.Добавить("infobase");
	ПараметрыЗапуска.Добавить("config");
	ПараметрыЗапуска.Добавить("import");
	//ПараметрыЗапуска.Добавить("--force");
	ПараметрыЗапуска.Добавить(""""+ИмяИсходногоФайлаОбработки+"""");
	ПараметрыЗапуска.Добавить("--out="""+ИмяФайлаОбработки+"""");
...
СборкаЧерезУтилитуАвтономногоСервераВнешнихОбработок = Ложь;
