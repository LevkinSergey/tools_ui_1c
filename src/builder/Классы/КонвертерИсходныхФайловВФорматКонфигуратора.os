...
Функция ВыполнитьКонвертацию() Экспорт
	...
	Лог.Информация("ЕДИ " +ПутьКФайлуCLI);

	СтрокаЗапуска = """"+ПутьКФайлуCLI+""" -v -data """+РабочийКаталогРИНГ+""" -command export --project """+КаталогИсходниковEDT+""" --configuration-files """+КаталогРезультатаКонвертации+""" -vmargs -Dfile.encoding=UTF8";
	...
	Лог.Информация("текстФайла:"+Текст.ПолучитьТекст());
	...
	КонецЕсли;
	Текст.Записать(ИмяИсполняемогоФайла,КодировкаТекста.UTF8);
	
	КодВозврата = Неопределено;
	ЗапуститьПриложение(ИмяИсполняемогоФайла,,Истина, КодВозврата);
